...
def index_delete(requset,account):
    member = member.objects.get(account=str(account))
    member.delete()
    response = HttpResponse("<script>alert('刪除成功');location.href='/member/index/aicoco'</script>")
    return response
...
def space(request,account):
    member = member.objects.get(account=str(account))
    return render(request,'member/space.html',locals())
...
def create(request): 
    if request.method == "POST":
        ...
        #比對資料庫帳號和email是否重複
        account_filter = member.objects.filter(account = account)
        email_filter = member.objects.filter(email = email)
        if len(account_filter)>0 : 
            return HttpResponse("<script>alert('帳號已有人使用,請重新輸入');location.href='/member/create'</script>")
        elif len(email_filter)>0 :  
            return HttpResponse("<script>alert('email已註冊過,請重新輸入');location.href='/member/create'</script>")  
        #驗證密碼是否輸入正確
        elif password != password2 or len(password) < 6 :
            return HttpResponse("<script>alert('密碼不得小於六位數或是比對不符,請重新輸入');location.href='/member/create'</script>")
        #生日不得為空值
        elif birth =="":
            return HttpResponse("<script>alert('請選取生日');location.href='/member/create'</script>")
        #是否同意條款及政策
        elif agree != "checked" :
            return HttpResponse("<script>alert('請勾選同意條款及政策');location.href='/member/create'</script>")
        #判定用戶是否為機器人
        elif result == False :
            return HttpResponse("<script>alert('未通過機器人驗證');location.href='/member/create'</script>")
        else :
            #產生亂數驗證碼
            ...
            #將資料寫進DB
            member.objects.create(account = account, email = email, password = password, birth = birth, grade = grade,vcode = tpassword)
            response = HttpResponse("<script>alert('帳號已新增,驗證碼已寄送信箱,請重新登入後進會員空間驗證');location.href='/member/grade'</script>")
...
def grade(request) :
    if request.method == "POST":
        ...
        member = member.objects.filter(account=user_account,email=user_email)
        if member :
            members = member.objects.get(account=str(user_account))
            #比對驗證碼是否相符
            if members.vcode == user_vcode :
                members.grade = "已驗證會員"
                members.save()
                return HttpResponse("<script>alert('信箱驗證成功');location.href='/member/login/'</script>")
            else :
                return HttpResponse("<script>alert('驗證失敗,請重新輸入');location.href='/member/grade/'</script>")
        else :
            return HttpResponse("<script>alert('帳號或email輸入錯誤,請重新輸入');location.href='/member/grade/'</script>")
    return render(request,'member/grade.html',locals())
...
def grade_forget(request) :
    if request.method == "POST":
        ...
        if member :
            ...
            #執行寄email密碼的動作
            send_email(tpassword,email,whoscall,account)
            return HttpResponse("<script>alert('新驗證碼已寄出');location.href='/member/grade/'</script>")
        else :
            return HttpResponse("<script>alert('帳號或email輸入錯誤,請重新輸入');location.href='/member/grade_forget'</script>")
    return render(request,'member/grade_forget.html',locals())
...
def update(request,account): 
    if request.method == "POST":
        ...
        if password =="" or len(password) < 6 :  
            return HttpResponse("<script>alert('密碼不得為空且不得小於6位數');location.href='/member/update/{}'</script>".format(account)) 
        else :
            #修改資料庫中的會員資料,並儲存
            ...
            member.job = job
            member.save()
            return HttpResponse("<script>alert('帳號修改成功');location.href='/member/space/{}'</script>".format(account))
    member = member.objects.get(account=str(account))
...
def delete(request,account): 
    ...
    #刪除DB會員資料
    member.delete()
    response = HttpResponse("<script>alert('刪除成功');location.href='/member/create'</script>")
    #刪除會員cookie
    response.delete_cookie("account")
    return response
...
def login(request):  
    if request.method == "POST":
        ...
        member = member.objects.filter(account=account,password=password).values("account")
        if member :
            members = member.objects.get(account=str(account))
        ...
        if member and (usergrade =="管理員" or usergrade =="已驗證會員") and result == True :
            ...
            response = HttpResponse("<script>alert('帳號未啟用，請先驗證email');location.href='/member/login'</script>")
        else:
            response = HttpResponse("<script>alert('帳號密碼輸入錯誤或未通過機器人驗證');location.href='/member/login'</script>")
        return response
...
def forget(request) :
    if request.method == "POST":
        ...
        members = member.objects.filter(account=account,email=email)
        if members :
            member = member.objects.get(account=str(account))
            #亂數編碼
            tpassword = random_code()
            ...
            #執行寄email密碼的動作
            send_email(tpassword,email,whoscall,account)
            response = HttpResponse("<script>alert('新密碼已寄出');location.href='/member/forget'</script>")
        else :
            response = HttpResponse("<script>alert('帳號或email不正確');location.href='/member/forget'</script>")
        return response
